...
class Geometry:
    ...
    def init_from_EZRT_header(self,  projection_headers: Tuple[str, ...], reco_header = None ,detector_shape = None,detector_spacing = None, swap_detector_axis: bool = True,**kwargs) -> None:
        ...
        if reco_header == None:
            ...
            if reco_header.voxel_size_z_in_um != 0 and reco_header.voxel_size_x_in_um != 0:
                self.parameter_dict['volume_spacing'] = np.asarray(
                    [reco_header.voxel_size_z_in_um/ (1000.0), reco_header.voxel_size_x_in_um/ (1000.0), reco_header.voxel_size_x_in_um/ (1000.0)]) *(scaling_factor*0.55) # *1.15
            elif reco_header.voxel_size_in_um != 0:
                self.parameter_dict['volume_spacing'] = np.full(shape=3, fill_value=header.voxel_size_in_um / 1000.0,
                                                            dtype=self.np_dtype)
        else:
        ...
        if detector_shape == None:
            ...
            self.parameter_dict['detector_shape'] = np.array(detector_shape)
            self.parameter_dict['detector_spacing'] = np.full(shape=2, fill_value=detector_spacing/ 1000.0,
                                                          dtype=self.np_dtype)  # np.array(header.pixel_width_in_um, dtype=self.np_dtype)
