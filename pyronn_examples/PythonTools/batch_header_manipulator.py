...
class BatchHeaderManipulator:
    ...
    def __init__(
        ...
        Args:
            folderpath: folder in which files with suffix are searched
            file_pattern: file pattern to search for (see glob)
            ezrt_header: header to add or overwrite file with (optional for FhG formats; if given in this case, this
                         input header is used instead of the files own header)
    ...
    def add_files_from_folder(self, folderpath: Path | str, ezrt_header: EzrtHeader | None = None):
        ...
        Args:
            folderpath: folder in which files with EZRT header are searched
            ezrt_header: header to add or overwrite file with (optional for FhG formats; if given in this case, this
                         input header is used instead of the files own header)
        """
        self.folderpath = Path(folderpath)
    ...
    def get_attribute_by_index(self, attribute: str, header_index: int):
        ...
        Args:
            attribute: name of header attribute
            header_index: specifies the header in the self.header list
    ...
    def change_attribute_by_index(
        self, attribute: str, value: int | float | str, header_index: int, save_headers: bool = False
    ):
        """
        ...
        Args:
            attribute: name of header attribute
            header_index: specifies the header in the self.header list
            value: the value, which gets assigned.
            save_headers: save the headers to hdd.
        """
        if header_index >= len(self.headers):
            raise ValueError('no header loaded or header index out of range')
    ...
    def get_attribute_all(self, attribute: str) -> list:
        ...
        Returns:
            A list of the specified attribute from all headers from th project
        """
        if len(self.headers) == 0:
    ...
    def save(self, overwrite: bool = True, new_folder_path: Path | str = 'files_with_header'):
        ...
        Args:
            overwrite: if True overwrites loaded files, if False a new folder is created to store manipulated files
            new_folder_path: path of new folder, if overwrite is False
        """
...
class BatchProjectionManipulator(BatchHeaderManipulator):
    ...
    def __init__(
        self,
        folderpath: Path | str,
        load_projection_data: bool = False,
        ...
        Args:
            folderpath: specifies the folder in which files with EZRT header are searched
            load_projection_data: state if projection data should be loaded to memory (needs a lot more memory) or read
                                  on demand (projection need to be loaded for every operation => more CPU load). Does
                                  only apply for image data, headers are always loaded.
            dtype: target data types of projections
            verbose: print debug output
        """
        super().__init__(folderpath, '*.raw', verbose=verbose)

        self.dtype: DTypeLike = dtype
        self.projection_data: list[np.ndarray] = []
    ...
    def __str__(self):
        return (
            f'--------- Projections @ path {self.folderpath}----------\n\n'
    ...
    def execute_function_by_index(
        ...
        Args:
            index: index of the function to execute
            target: target function to call
            args: positional arguments of passed function as tuple
            kwargs: keyword arguments of passed function as dict
            save: flag if projections should be directly saved (mandatory if projections are not loaded beforehand)
            overwrite: flag if projections should be overwritten
        """
        ...
        if self._projections_loaded:
            ...
            for i, (modified_header, modified_image) in enumerate(
                self.iterate_function(target, args=args, kwargs=kwargs, save=False, overwrite=overwrite)
            ):
                if i != index:
                    continue
    ...
    def execute_function_all(
        ...
        for i, (header, image) in enumerate(
            self.iterate_function(target, args=args, kwargs=kwargs, save=save, overwrite=overwrite)
        ):
    ...
    def iterate_function(
        ...
        Args:
            ...
            args: positional arguments of passed function
            kwargs: keyword arguments of passed function
            save: flag if projections should be directly saved
            overwrite: flag if projections should be overwritten

        Yields:
            current header and projection data
        """
        if args is None:
            args = ()
        if kwargs is None:
        ...
        for header, projection, filepath in zip(self.headers, projection_data, self._files_with_suffix_in_path):
            if self.verbose:
                print('>', end='', flush=True)

            # execute function (copy projection data, to avoid read-only issue)
            modified_header, modified_projection = target(header, np.copy(projection), *args, **kwargs)
            if save:
                if not overwrite:
                    filepath = filepath.parent / function_name / filepath.name
                py2raw(modified_projection.astype(self.dtype), filepath, input_header=modified_header)

            yield modified_header, modified_projection
    ...
    def save(
        self,
        overwrite: bool = True,
        new_folder_path: Path | str = 'files_with_header',
        ...
        Args:
            overwrite: if True overwrites loaded projections, if False a new folder is created to store manipulated
                       projections
            new_folder_path: name of new folder, if overwrite is False
            projection_prefix: prefix of newly saved projections (if overwrite is False)
        """
        if overwrite:
            for projection_data, header, filepath in zip(
                self.projection_data, self.headers, self._files_with_suffix_in_path
            ):
                py2raw(projection_data.astype(self.dtype), filepath, input_header=header)
            return
        ...
        for i, (projection_data, header, filepath) in enumerate(
            zip(self.projection_data, self.headers, self._files_with_suffix_in_path)
        ):
...
class BatchImageConverter(BatchHeaderManipulator):
    ...
    Supported image formats can be seen in ImageFormat enum.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            ezrt_header: header to add or overwrite file with (optional for FhG formats; if given in this case, this
                         input header is used instead of the files own header)
            imageformat_out: output image as ImageFormat
    ...
    def save(self, new_folder_path: Path | str | None = None):
        ...
        Args:
            new_folder_path: target folder path (if None converted images are saved in the same folder as source)
        """
