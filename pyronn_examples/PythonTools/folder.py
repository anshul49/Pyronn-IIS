...
def get_postfix_number(filename: Path | str):
    ...
    postfix_result = re.search('_[0-9]+(?!.*_[0-9]+)', filename)
    result = re.search(r'\.+(?!.*\.)', filename)
    if postfix_result is None or result is None:
        raise IndexError('no valid postfix')
    if postfix_result.end() != result.end() - 1:
        raise IndexError('postfix number does not have the correct form ')
    return int(re.findall('_[0-9]+(?!.*_[0-9]+)', filename)[0][1:])
...
def get_folder_state(folderpath: Path | str, fileprefix: str, raise_exception: bool = False) -> tuple[int, int]:
    ...
    Args:
        ...
        fileprefix: prefix of projection files
        raise_exception: if True, Exceptions are thrown instead of the 0, 0 return value in an error case

    Returns
        number of projections found, number of planned projections
    """
    ...
    filtered_filenames = list(
        filter(lambda filename: re.match(fileprefix + r'_[0-9]+', filename.name), folderpath.iterdir())
    )
