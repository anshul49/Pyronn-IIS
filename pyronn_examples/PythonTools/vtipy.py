...
# All Rights Reserved.
#
# This file is part of the PythonTools project (used to be part of the FileIO project).
#
...
# exclude from type checking as vtk has no stubs
# type: ignore
...
def py2vti(volume_array: np.ndarray, filepath: Path | str, image_data_writer_callback: Callable | None = None):
    ...
    Args:
        ...
        filepath: file path where new .rek file is saved
        image_data_writer_callback: callback to manipulate vtkXMLImageDataWriter options; prototype: func(writer)
    """
    # vtk wants 3d
    ...
    vtk_image_data.SetSpacing(
        [
            1,
        ]
        * volume_array.ndim
    )
    vtk_image_data.SetOrigin(
        [
            0,
        ]
        * volume_array.ndim
...
def vti2py(filepath: Path | str) -> np.ndarray:
    ...
    Args:
        filepath: file path to .vti file

    Returns:
        2D numpy array representation of .vti file

    Raises:
        FileNotFoundError: if the given path is no file
    """
