...
def example_fan_2d_shortscan():
    ...
    angular_range =  np.pi + 2 * np.arctan(((detector_shape[0] - 1) / 2.0 * detector_spacing[0]) / sdd)
    ...
    redundancy_weights = torch.tensor(weights.parker_weights_2d(geometry),dtype=torch.float32).cuda()
    sinogram_redun_weighted = sinogram * redundancy_weights

    # Filtering: Create 2D Filter and pointwise multiply
    reco_filter = torch.tensor(filters.ram_lak_2D(geometry)).cuda()
